...
11. Apply `orizon/policies/ORIZON-ECC-SUPREMACY-LEGACY-PRECEDENCE.md`: ECC is the current governing authority; legacy Strategy, Central Office, Hub, Birth Lifecycle, superseded Bootstrap, Official Resume, and historical governance hooks are non-blocking unless explicitly re-adopted by current ECC.
12. Apply `orizon/policies/ORIZON-CONTINUOUS-AUTONOMOUS-EXECUTION-POLICY.md`: once a mission is authorized, continue autonomously through fixable failures until acceptance criteria are met or a genuine human gate is reached. Verify instead of asking, fix instead of reporting, and never create artificial approval checkpoints.
