...
Use `orizon-engineering-profile` and the relevant standards, especially `GO-LIVE-PRODUCTION.md`.

Follow: Database -> Security -> Backend -> Frontend -> Tests -> Observability -> Staging -> Production.
Identify the exact release commit/artifact and deployment target.
Run the repository's real build, test and critical-flow validations.
Verify migrations, secrets, environment configuration, external integrations and rollback.
...
Return GO only when critical blockers are cleared; otherwise return the exact blockers and next actions.
